# Fine-Tuning T5 Model for Text To SQL using Transformers Library
...
The code uses the [Clinton/texttosqlv2_25000_v2](https://huggingface.co/datasets/Clinton/texttosqlv2_25000_v2) dataset, which is a popular dataset for question answering tasks. The dataset consists of questions posed by crowdworkers on a set of Wikipedia articles, where the answer to every question is a segment of text, or span, from the corresponding reading passage.
...
* split: The split of the dataset to load. In this case, it is set to **"train[:10000]"** for training data, **"train[10000:12500]"** for validation data, and **"train[22500:25000]"** for test data.
